...
public static class NameHelperGenerator
{
    ...
    public static IAsyncStream<TMessage> GetStream<THub, TMessage>(IClusterClient clusterClient, string streamProviderName, string streamName) where THub : Hub
    {
    ...
    public static IAsyncStream<TMessage> GetStream<TMessage>(string hub, IClusterClient clusterClient, string streamProviderName, string streamName)
    {
        var streamProvider = clusterClient.GetStreamProvider(streamProviderName);
        return GetStream<TMessage>(hub, streamProvider, streamName);
    }
